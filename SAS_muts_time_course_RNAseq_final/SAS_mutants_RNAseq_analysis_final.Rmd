...
* Made all.my.category.expression.pattern.v9.png with ORA between custom categories (from castom categoreis vs SOM clusters) and misexpressed genes in each mutant (052916). needs to use GOseq.overlapTable instead of overlapTable in future.
* My.category.expression.pattern.v10.png with "A" and "B" panel (learned geom_blank(), extract legend, etc) (060216)
* needs to use all genes for GOseq.overlapTable analysis
* WGCNA vs castum category (including PCC1up and down) with permutation 10000 (in Whitney, 060216) (default is 2000)
* fix flipped axis label in a heatmap
* change heatmap color ("green" + "magenta" into "white" and "mangenta") (061816)
* final version for submission (080316)
...
homedir<-"/Volumes/Toshiba_CANVIO2/not synched with my computer-selected/Nozue2016_SAStranscriptome_scripts/SAS_muts_time_course_RNAseq_final"
} # customize homedir
setwd(homedir)
...
## eliminating libraries with wrong genotypes
```{r eval=FALSE}
toc<-read.table("../../Nozue2016_SAStranscriptome_data/input/SAS_counts_merged_updated_final.bam.tsv",header=TRUE) # use SAS_counts_merged_updated.bam.tsv (081514)
# library(edgeR)
# remove * row and move gene column into rowname
rownames(toc)<-toc$gene
toc<-toc[!toc$gene=="*",-1] 
#head(toc)
label<-names(toc)
...
#
counts<-toc
save(counts,file="../../Nozue2016_SAStranscriptome_output/output/counts.Rdata")
#load("../../Nozue2016_SAStranscriptome_output/output/counts.Rdata")
...
Genotype<-levels(as.factor(samples.nolow5$genotype))
Genotype<-Genotype[!(Genotype=="11"|Genotype=="12"|Genotype=="18"|Genotype=="16")]
for(i in Genotype[1]) { # why I eliminate genotype 1 and 10???
  print(paste("Genotype is ",i))
  ...
  dge.nolow.s.batch.glm <- estimateGLMCommonDisp(dge.nolow.s,design.nolow.s.batch, verbose=TRUE) 
  dge.nolow.s.batch.glm <- estimateGLMTrendedDisp(dge.nolow.s.batch.glm,design.nolow.s.batch, verbose=TRUE)
  dge.nolow.s.batch.glm <- estimateGLMTagwiseDisp(dge.nolow.s.batch.glm,design.nolow.s.batch) 
  ...
  save(DE.batch.anytime,file=paste("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/DE.batch.anytime_genotype_wo_outlier_wo16h",i,".Rdata",sep="")) # file name fixed (122114)    
}
```
# check library by MDS plot
```{r eval=FALSE}
########
# under construction (060415)
########
setwd(homedir)
setwd("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h") # atmosphere
load("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/summary4.Rdata") # no file?
summary4.s<-summary4[rowSums(summary4 > 5) >= 30,]
dim(summary4.s) #15800   184
scale(t(summary4.s[1:10,]))
summary4.s.melt<-melt(t(scale(t(summary4.s[1:100,]))))
qplot(value,data=summary4.s.melt,geom="histogram",binwidth=1) + facet_wrap(~Var1)
qplot(value,data=summary4.s.melt,geom="histogram",binwidth=0.1) + facet_wrap(~Var1)
library.mds<-as.data.frame(cmdscale(dist(scale(t(summary4.s)))))
library.mds$gt<-gsub("([[:digit:]]+)(H|L)([[:digit:]]+)","\\1",rownames(library.mds))
library.mds$trt<-gsub("([[:digit:]]+)(H|L)([[:digit:]]+)","\\2",rownames(library.mds))
library.mds$time<-gsub("([[:digit:]]+)(H|L)([[:digit:]]+)","\\3",rownames(library.mds))
library.mds$samplename<-rownames(library.mds)                
p<-ggplot(data=library.mds,aes(x=V1,y=V2,label=samplename,color=factor(time))) + geom_text()
p<-p + facet_wrap(~gt,nrow=3)
ggsave("library.MDSplot.pdf",width=20,height=11) # 1 and 23 are similar. At least 23 are not natural accession (20-27).
```

# summary of DE genes 
* (053014 - 053114): revised version (081514 - );more revision (060415 - )
```{r eval=FALSE}
# DEfiles<-list.files(path="/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp_old/edgeR/outlier_wo16h_whitney/",pattern="DE.batch.anytime")
...
for(i in DEfiles[-1]) {
  ...
  if(dim(DE.batch.anytime)[1]>1) {
    rownames(DE.batch.anytime)<-paste(rownames(DE.batch.anytime),gsub("(DE.batch.anytime_genotype_wo_outlier_wo16h)([[:print:]]+)(.Rdata)","\\2",i),sep=".")
    temp<-rbind(temp,DE.batch.anytime)  } # fixed typo (052916)
  else next
}
...
getwd()
save(DGEsummary.edgeR.response,file="DGEsummary.edgeR.response.Rdata")
# genotype conversion 
conversion.table<-data.frame(num=1:27, genotype=c("Col","AT5G02540_1","hy5","jar1","kat1_2","phyB","pif45","spt_11","yuc2589","PAR1_RNAi09","coi1_16","phyAB","pif3","mida9_4","AT5G59010_2","sto","aos","argos","co_9","Blh_1","Jea","Shahdara","Col_0","Cvi_0","Bur_0","Oy_0","Ita_0"))
load("samples.nolow5.Rdata")
samples.nolow5
...
SAS.expression.vst<-read.csv("../../Nozue2016_SAStranscriptome_data/input/kazu.SAS.expression.VST.SMALL.112614.csv") # new transformed data by DEseq2 (done by J, see VST.R)
...
sum(as.integer(SAS.expression.vst.s.kazu$cv>3))/dim(SAS.expression.vst.s.kazu)[1] #[1] 0.4011866
SAS.expression.vst.s.kazu.largeCV<-SAS.expression.vst.s.kazu[SAS.expression.vst.s.kazu$cv>3,]
dim(SAS.expression.vst.s.kazu.largeCV) #[1] 7438  403
...
dim(summary.vst.response.kazu.largeCV) # 7438   60
...
#length(DGEgenelist.edgeRglm) #418 
DGEgenelist.edgeRglm<-unique(gsub("([[:print:]]+)(.[[:digit:]]+)(\\.)([[:digit:]]+)","\\1",rownames(DGEsummary.edgeR.response))) # 
length(DGEgenelist.edgeRglm) #369
...
DGE.summary.vst.response.kazu.largeCV[rownames(DGE.summary.vst.response.kazu.largeCV)=="AT4G16780",] #ATHB2 
```
...
load("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/DGE.summary.vst.response.kazu.largeCV.Rdata") # this was updated... (052916)
...
write.csv(SOMclusters.description,"../../Nozue2016_SAStranscriptome_output/figs_tables/TableS3_SOMclusters.description.csv")
...
p <- p + coord_flip()
p
ggsave("boxplot.node.genotype3.4.noscaling_vst_Col.pdf",width=11,height=2,unit="in",path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
for(x in clusters[!clusters %in% c("1","3","9","11","12")]) { # skip clusters with no GO enrichment
...
write.csv(SOM.Col.cluster.GOseq.summary,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS5_SOM.Col.cluster.GOseq.summary.csv")
```
...
# shade
#setwd("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/genotype_specific5")
DE.genotype.batch.shade.files<-list.files(pattern="DE.genotype.batch.shade",path="../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/genotype_specific5")
...
# SOM clusters
load("../../Nozue2016_SAStranscriptome_output/output/newdata.vst.Rdata") # 022215 # this is wrong. cluster10 has only 1 gene? (060716)
newdata.vst.Col<-newdata.vst[newdata.vst$gt==1,]
...
write.csv(DE.genotype.batch.genes.SOMclusters.description,"../../Nozue2016_SAStranscriptome_output/figs_tables/TableS6_DE.genotype.batch.genes.SOMclusters.description.csv")
...
overlap.GOseq<-GOseq.overlapTable(test[,30],test[,i],genes=test$AGI,permutation=10000)
...
save(overlap.GOseq.melt.summary, file="../../Nozue2016_SAStranscriptome_output/output/overlap.GOseq.melt.summary.SOM.DEgenes.060416.Rdata")
load("../../Nozue2016_SAStranscriptome_output/output/overlap.GOseq.melt.summary.SOM.DEgenes.060416.Rdata")
# graph
...
setwd("/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp")
ggsave(q4b,file="Fig4_Col.SOM.clusters.vs.GenotypeSpecific.genes.table.s.2.GOseq.061816.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
conversion.table
# custom categories
load("/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp/hormone.responsiveness3.Rdata") #new version. check if new version is identical to this. copy the file to this location
...
newdata.vst.Col<-newdata.vst[newdata.vst$gt==1,] # this is wrong... . Replaced with old one from "SAS_mutants_time_course_RNAseq2_mycomp_bk" (060716)
# merge with SOM clusters
...
write.csv("/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp_out/hormone.responsiveness.SOMclusters.description.csv")
# in whitney (060616)
test<-read.csv("../../Nozue2016_SAStranscriptome_output/output/TableS4_hormone.responsiveness.SOMclusters.csv")
test<-test[,-1]
#
#test<-hormone.responsiveness.SOMclusters
# overlap table
i<-2
overlap<-GOseq.overlapTable(test[,28],test[,i],genes=test$AGI,permutation=10000)
...
save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.060616.Rdata") # whitney. redoing with correct new table. (060716)
####### under construction #########
setwd("/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp_output")
load("overlap.melt.summary.SOM.custom.GOseq.060616.Rdata")
...
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.061816.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.061816.png",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
```{r eval=FALSE}
load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.Rdata")
setwd("/Volumes/Data8/NGS_related/Arabidopsis_analysis/SAS_muts_time_course_RNAseq2_mycomp")
load("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/samples.nolow5.Rdata")
Genotype<-levels(as.factor(samples.nolow5$genotype))
...
p
ggsave("boxplot.node.WGCNAmodule.salmon2.png",width=4,height=3,unit="in",path="../../Nozue2016_SAStranscriptome_output/figs_tables") # 
...
moduleColors.files<-list.files(pattern=".largeCV.RData")
...
save(SAS.expression.vst.s.kazu.largeCV.modules,file=",path="../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules.Rdata") 
```
...
WGCNA.SOMcluster.Col<-merge(SAS.expression.vst.s.kazu.largeCV.modules[,c("AGI","1")],newdata.vst[newdata.vst$gt=="1",c("AGI","ssom3.4.unit.classif")],by="AGI",all=TRUE) # 
...
q4 # magenta indicates pvalue is smaller than 0.01.
ggsave(q4,file="Col.SOMcluster.VS.Col.WGCNA.modules.table.pdf",width=8,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
```
...
DE.genotype.batch.genes.WGCNAmodules<-DE.genotype.batch.genes.WGCNAmodules[,-c(1:2)]
# adding gene description
TAIR10_gene_descriptions<-read.csv("../../Nozue2016_SAStranscriptome_data/input/TAIR10_functional_descriptions.csv") 
TAIR10_gene_descriptions$AGI<-gsub("([[:print:]]+)(.[[:digit:]]+)","\\1",TAIR10_gene_descriptions$"Model_name")
...
write.csv(DE.genotype.batch.genes.WGCNAmodules.description,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS10_DE.genotype.batch.genes.WGCNAmodules.description.csv") #Table S6 in manuscript
# in whitney
test<-read.csv("../../Nozue2016_SAStranscriptome_output/figs_tables/TableS10_DE.genotype.batch.genes.WGCNAmodules.description.csv")
test<-test[,-1]
dim(test) # 8683   32
test<-DE.genotype.batch.genes.WGCNAmodules
# overlap table (revsised into GOseq.overlapTable; 052616)
# with ,permutation=10000 (060316)
i<-2
overlap<-GOseq.overlapTable(test[,30],test[,i],genes=test$AGI)
overlap.melt<-melt(overlap$pTable,id=1,permutation=10000) #
overlap.melt$count<-melt(overlap$countTable,id=1)$value
overlap.melt.summary<-overlap.melt[!overlap.melt$Var2=="0",]
for(i in 3:29) {
overlap<-GOseq.overlapTable(test[,30],test[,i],genes=test$AGI,permutation=10000)
overlap.melt<-melt(overlap$pTable,id=1) #
overlap.melt$count<-melt(overlap$countTable,id=1)$value
overlap.melt<-overlap.melt[!overlap.melt$Var2=="0",]
overlap.melt.summary<-rbind(overlap.melt.summary,overlap.melt)
...
# graph
overlap.melt.summary.s<-overlap.melt.summary[!overlap.melt.summary$Var2 %in% c("bsk5_1.sun","bsk5_1.shade","AT5G02540_1.sun","AT5G02540_1.shade"),]
save(overlap.melt.summary.s,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.s.WGCNA.mut.GOseq.Rdata")

# reading and cleaning overlap table
# load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.s.Rdata")
load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.s.WGCNA.mut.GOseq.Rdata") # calculated in whitney (052616)
...
ggsave(q4,file="Fig7_Col.WGCNA.modules.vs.GenotypeSpecific.genes.table.s.GOseq061816.pdf",width=11,height=8,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
write.csv(WGCNA.Col.module.GOseq.summary,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS8_WGCNA.Col.module.GOseq.summary.csv") # supplementary table S8
```
...
i<-2
overlap<-GOseq.overlapTable(test[,"WGCNAmodules"],test[,i],genes=test$AGI,permutation=10000) # default permutaiton is 2000 in goseq(method="Sampling")
overlap.melt<-melt(overlap$pTable,id=1) #
overlap.melt$count<-melt(overlap$countTable,id=1)$value
...
g_legend<-function(a.gplot){ 
  ...
  return(legend)} 
legend <- g_legend(my.category.heatmap.graph[[1]]+ guides(fill = guide_legend(keywidth = 3, keyheight = 1)+theme(legend.position="bottom"))) 
#
library(cowplot)
...
### combine all Fig. 5A, B, C

my.category.heatmap.all3 <- plot_grid(my.category.heatmap.graph[[1]]+theme(legend.position="none"),legend ,ggplot()+geom_blank(),my.category.heatmap.graph[[17]],my.category.heatmap.graph[[19]],my.category.heatmap.graph[[3]],my.category.heatmap.graph[[4]],my.category.heatmap.graph[[5]],my.category.heatmap.graph[[14]],my.category.heatmap.graph[[10]],my.category.heatmap.graph[[6]],my.category.heatmap.graph[[13]],my.category.heatmap.graph[[7]],my.category.heatmap.graph[[8]],my.category.heatmap.graph[[9]],my.category.diff.heatmap.graph[[17]],my.category.diff.heatmap.graph[[19]],my.category.diff.heatmap.graph[[3]],my.category.diff.heatmap.graph[[4]],my.category.diff.heatmap.graph[[5]],my.category.diff.heatmap.graph[[14]],my.category.diff.heatmap.graph[[10]],my.category.diff.heatmap.graph[[6]],my.category.diff.heatmap.graph[[13]],my.category.diff.heatmap.graph[[7]],my.category.diff.heatmap.graph[[8]],my.category.diff.heatmap.graph[[9]],
                      labels=c("A","","","B","","","","","","","","","","","","C","","","","","","","","","","",""),
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=9,
                      label_size = 50)
#
setwd("../../Nozue2016_SAStranscriptome_output/figs_tables/")
save_plot("Fig_5_all.my.category.expression.pattern.v11b.png", my.category.heatmap.all3,
          ncol = 3, # we're saving a grid plot of 2 columns
          nrow = 9, # 
          base_aspect_ratio = 2.5,
          base_height=6,
...
save_plot("Fig_5_all.my.category.expression.pattern.v11b.pdf", my.category.heatmap.all3,
          ncol = 3, # we're saving a grid plot of 2 columns
          nrow = 9, # 
          base_aspect_ratio = 2.5,
          base_height=6,
...
# having space between A, B, C 
## 
my.category.heatmap.A <- plot_grid(my.category.heatmap.graph[[1]]+theme(legend.position="none"),legend ,#ggplot()+geom_blank(),
                      #labels=c("A","",""),
                      rel_widths=c(1,2),
                      rel_htights=c(1,4),
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=1,
                      label_size = 50)
my.category.heatmap.B <- plot_grid(my.category.heatmap.graph[[17]],my.category.heatmap.graph[[19]],my.category.heatmap.graph[[3]],my.category.heatmap.graph[[4]],my.category.heatmap.graph[[5]],my.category.heatmap.graph[[14]],my.category.heatmap.graph[[10]],my.category.heatmap.graph[[6]],my.category.heatmap.graph[[13]],my.category.heatmap.graph[[7]],my.category.heatmap.graph[[8]],my.category.heatmap.graph[[9]],
                      #labels=c("B","","","","","","","","","","","","C","","","","","","","","","","",""),
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=4,
                      label_size = 50)
my.category.heatmap.C <- plot_grid(my.category.diff.heatmap.graph[[17]],my.category.diff.heatmap.graph[[19]],my.category.diff.heatmap.graph[[3]],my.category.diff.heatmap.graph[[4]],my.category.diff.heatmap.graph[[5]],my.category.diff.heatmap.graph[[14]],my.category.diff.heatmap.graph[[10]],my.category.diff.heatmap.graph[[6]],my.category.diff.heatmap.graph[[13]],my.category.diff.heatmap.graph[[7]],my.category.diff.heatmap.graph[[8]],my.category.diff.heatmap.graph[[9]],
                      #labels=c("C","","","","","","","","","","",""),
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=4,
                      label_size = 50)
my.category.heatmap.ABC<-plot_grid(my.category.heatmap.A,my.category.heatmap.B,my.category.heatmap.C,
                                   labels=c("A","B","C"),
                                   rel_widths=c(2,1,1),
                                   rel_heights=c(1,3,3),
                                   ncol=1,nrow=3,label_size = 50,
                                   vjust = -0.1, hjust=-0.5)

setwd("../../Nozue2016_SAStranscriptome_output/figs_tables/")
 save_plot("Fig_5_all.my.category.expression.pattern.v11c.png", my.category.heatmap.ABC,
...
load("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/DGEsummary.edgeR.response.Rdata")
