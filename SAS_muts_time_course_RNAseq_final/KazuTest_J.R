...
expr.melt.merge$gt <- relevel(expr.melt.merge$gt,ref="1")

table(expr.melt.merge$gt,expr.melt.merge$time)

mutants <- levels(expr.melt.merge$gt)[-1] #1 is Col (Gt
...
results <- lapply(mutants,function(gt) { 
  sapply(category.names,function(cat.name) {
    tmp <- expr.melt.merge[(expr.melt.merge$gt=="1" | expr.melt.merge$gt==gt) & expr.melt.merge$my.category==cat.name,]
...
results.df.adj.Julin<-results.df.adj
